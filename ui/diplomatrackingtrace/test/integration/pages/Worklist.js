sap.ui.define([
	...
	"diploma/tracking/overview/test/integration/pages/shareOptions"
			...
			assertions: jQuery.extend({
				...
				theTableShowsOnlyObjectsWithTheSearchStringInTheirTitle: function() {
					this.waitFor({
						...
						check: allItemsInTheListContainTheSearchTerm,
						success: function() {
							QUnit.ok(true, "Every item did contain the title");
						},
				...
				theTableHasEntries: function() {
					return this.waitFor({
						...
						success: function() {
							QUnit.ok(true, "The table has entries");
						},
				...
				theTableShouldHaveAllEntries: function() {
					...
					return this.waitFor({
						...
						success: function(oTable) {
							QUnit.strictEqual(oTable.getItems().length, iExpectedNumberOfItems, "The growing Table has " + iExpectedNumberOfItems +
								" items");
						},
				...
				iShouldSeeTheNoDataTextForNoSearchResults: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function(oTable) {
							QUnit.strictEqual(oTable.getNoDataText(), oTable.getModel("i18n").getProperty("worklistNoDataWithSearchText"),

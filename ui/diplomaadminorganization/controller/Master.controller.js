...
sap.ui.define([
	...
	return BaseController.extend("diploma.admin.organization.controller.Master", {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("diploma.admin.organization.view.ViewSettingsDialog", this);
		...
		/** Opens the role management dialog. */
		onOpenRoleDialog: function() {
		    this.byId("dlgRole").open();
		},
		
		/** Closes the role management dialog. */
		onCloseRoleDialog: function() {
		    this.byId("dlgRole").close();
		},
		
		/** Adds a new role. */
        onAddRole: function() {
            this.getModel().create("/Roles", {
                id:         "",
                name:       this.byId("inpRoleName").getValue(),
                contact:    parseInt(this.byId("selRoleContact").getSelectedKey(), 10)
            });
        },
        ...
        */
        ...
        /** 
        * Filters the role list by using the search criteria.
        * @param    {Event} oEvent  The event object.
        * @returns  {void}
        */
        onRoleSearch: function(oEvent) {
			var sQuery = oEvent.getParameter("query"),  
			    aFilters = sQuery ? [new Filter("name", FilterOperator.Contains, sQuery)] : [];
            this.byId("lstRoles").getBinding("items").filter(aFilters);
        },
